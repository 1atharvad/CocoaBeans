...
function getDatabase() {
    var data = JSON.parse(sessionStorage.getItem('custom-box'));
    if (data !== null) {
        createDropList(data.size);
        createDraggableList(data.selectables);
    }
    firebase.database().ref("products/custom-box").on('value', (snapshot) => {
        if (JSON.stringify(data) !== JSON.stringify(snapshot.val())) {
            sessionStorage.setItem('custom-box', JSON.stringify(snapshot.val()));
...
function createDraggableList(products) {
    const dragItems = document.querySelector('#draggable-0');
    
    document.querySelector('#draggable-items').innerHTML = dragItems.outerHTML;
    for (var i=0; i<Object.keys(products).length; i++) {
        document.querySelector('#draggable-items').innerHTML += dragItems.outerHTML.replace('draggable-0', `draggable-${i+1}`);
        document.querySelector(`#draggable-${i+1} img`).src = `/images/${products[`product-${i+1}`].image}`;
    }
} 
